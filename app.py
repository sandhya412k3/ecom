...
def admincreate():
    if request.method=='POST':
        ...
        agred=request.form['agree']
        otp=genotp()
        admin={'username':username,'useremail':useremail,'password':password,'address':address,'agree':agred,'otp':otp}
        subject=f'OTP for Ecom admincreate'
        body=f'use the otp {otp}'
        send_mail(to=useremail,body=body,subject=subject)
        flash(f'OTP has sent to given email {useremail}')

        return redirect(url_for('otpverify',endata=entoken(data=admin)))
...
def otpverify(endata):
    try:
        ...
        print(f'Error in dcode admin ')
        flash('could not verify otp')
        return redirect(url_for(admincreate))
    else:
        if request.method=='POST':
            uotp=request.form['otp']
            # udata=request.form['otp']
            if uotp==ddata['otp']:
                salt=bcrypt.gensalt()
                hash=bcrypt.hashpw(ddata['password'].encode('utf-8'),salt)
                try:
                    cursor=mydb.cursor()
                    cursor.execute('insert into admin(username,adminemail,password,address,agree) values(%s,%s,%s,%s,%s)',[ddata['username'],ddata['useremail'],hash,ddata['address'],ddata['agree']])
                    mydb.commit()
                except Exception as e:
                    print(f'Error is {e}')
...
def adminlogin():
    if not session.get('admin'):
        if request.method=='POST':
            ...
            try:
                cursor=mydb.cursor()
                cursor.execute('select count(*) from admin where adminemail=%s',[useremail])
                email_count=cursor.fetchone()
                if email_count[0]==1:
                    cursor.execute('select password from admin where adminemail=%s',[useremail])
...
def viewallitems():
    if session.get('admin'):
        try:
            ...
            return render_template('viewall_items.html',items=items)
    else:
...
def adminprofile():
    if session.get('admin'):
        try:
            cursor=mydb.cursor()
            cursor.execute('select username,address,phone_no,profilepic from admin where adminemail=%s',[session.get('admin')])
            ...
            if request.method=='POST':
                ...
                if profilepic.filename!='':
                    filename=genotp()+'.'+profilepic.filename.split('.')[-1]
                    path=os.path.abspath(__file__)
                    print(path)
                    dirpath=os.path.dirname(path)
                    print(dirpath)
                    static_path=os.path.join(dirpath,'static')
                    print(static_path)
                    profilepic.save(os.path.join(static_path,filename))
                else:
                    ...
                    # print(static_path)
                    # profilepic.save(os.path.join(static_path,filename))
                cursor.execute('update admin set username=%s,address=%s,phone_no=%s,profilepic=%s where adminemail=%s',[username,address,phone_no,filename,session.get('admin')])
                mydb.commit()
                flash('successfully updated')
                return redirect(url_for('adminprofile'))


            return render_template('adminupdate.html',admin=stored_details)
...
def success():
    ...
    order_signature = request.form.get("razorpay_signature", "").strip()
    item_id = request.form.get("itemid", "").strip()
    name = request.form.get("name", "").strip()
    item_qyt = request.form.get("qyt", "0").strip()
    total_amount = request.form.get("total_amount", "0").strip()

    if not item_qyt.isdigit():
        flash("Invalid quantity received. Cannot process payment.")
        return redirect(url_for('index'))

    item_qyt = int(item_qyt)
    ...
    try:
        ...
        item = cursor.fetchone()
        new_quantity = item[3] - item_qyt

        cursor.execute('UPDATE items SET quantity = %s WHERE itemid = uuid_to_bin(%s)', [new_quantity, item_id])

        cursor.execute('SELECT address FROM users WHERE useremail = %s', [session.get('user')])
        user_data = cursor.fetchone()
        user_address = user_data[0] if user_data else 'Not Provided'

        cursor.execute(
            'INSERT INTO orders(item_name, total_amount, quantity, payment_by, address) VALUES (%s, %s, %s, %s, %s)',
            [name, total_amount, item_qyt, session.get('user'), user_address]
        )
        mydb.commit()
        cursor.close()
        ...
        flash('Something went wrong during payment processing.')
        return redirect(url_for('index'))
...
def readreview(itemid):
    try:
        cursor = mydb.cursor()
        cursor.execute('SELECT review, rating, user FROM reviews WHERE itemid = uuid_to_bin(%s)', [itemid]) 
